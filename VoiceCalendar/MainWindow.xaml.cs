...
public class AddEventDialog : Window
{
    ...
    public AddEventDialog(DateTime defDate)
    {
        ...
        g.Children.Add(Lbl("事件名称", 0, 0, 4));
        _tb = Tb(""); Grid.SetRow(_tb, 1); g.Children.Add(_tb);

        g.Children.Add(Lbl("日期", 12, 0, 4));
        _dp = new DatePicker { SelectedDate = defDate, FontSize = 14 }; Grid.SetRow(_dp, 3); g.Children.Add(_dp);

        g.Children.Add(Lbl("时间 (HH:mm，留空=全天)", 12, 0, 4));
        _tm = Tb("", 80); Grid.SetRow(_tm, 5); g.Children.Add(_tm);
...
public class EditEventDialog : Window
{
    ...
    public EditEventDialog(CalendarEvent ev)
    {
        ...
        g.Children.Add(Lbl("事件名称", 0, 0, 4));
        _tb = Tb(ev.Title); Grid.SetRow(_tb, 1); g.Children.Add(_tb);

        g.Children.Add(Lbl("日期", 12, 0, 4));
        _dp = new DatePicker { SelectedDate = ev.EventDate, FontSize = 14 }; Grid.SetRow(_dp, 3); g.Children.Add(_dp);

        g.Children.Add(Lbl("时间 (HH:mm)", 12, 0, 4));
        _tm = Tb(ev.EventTime ?? "", 80); Grid.SetRow(_tm, 5); g.Children.Add(_tm);
